...
class BilatGossipDataParallel(Module):
    ...
    def __init__(self, module, device_ids=None, master_addr=None,
                 ...
                 nesterov=True, verbose=True, network_interface_type=None, nprocs_per_node=1):
        ...
        self.train_write_flag = threading.Event()  # signal train-proc write event
        self.gossip_read_flag = threading.Event()  # signal gossip-proc read event
        self.gossip_update_flag = threading.Event()  # signal 2 gossip-proc need update     
        self._lr = mp.Value('f', lr, lock=self.gossip_lock)
    ...
    def _transfer_grads(self):
        ...
        #self.logger.debug('ad_psgd line 236 : before gossip_read_flag.wait()')
        self.gossip_read_flag.wait()
        #self.logger.debug('ad_psgd line 238 : after gossip_read_flag.wait()')
        ...
        #self.logger.debug(f'ad_psgd line 251, {self.train_write_flag.is_set()}')
        return True
    '''
    @staticmethod
    def _gossip_target(dist_config, gossip_enable_flag, train_write_flag,
        ...
        with torch.no_grad():
            ...
            logger.debug(f'ad_psgd line 325')

            # gossip loop
            while True:
                # we may be asked to hold off on gossip for some time
                #logger.debug(f'ad_psgd line 329')
                ...
                #logger.debug(f'ad_psgd line 339, {train_write_flag.is_set()}')
                if train_write_flag.is_set():
                    ...
                    #logger.debug('ad_psgd line 349 : before gossip_read_flag.set()')
                    gossip_read_flag.set()
                    #logger.debug('ad_psgd line 351 : after gossip_read_flag.set()')
                ...
                try:
                    ...
                    if completed:
                        #logger.debug(f'ad-psgd.py line 370 : {completed}')
                        with gossip_lock:
                            #logger.debug(f'ad-psgd.py line 372 : {gossip_lock}')
                            for p, g in zip(
                                    gossip_params, unflatten_tensors(
                                        in_msg, gossip_params)):
                                p.data.add_(g.to(p.device)).mul_(0.5)
                    #logger.debug(f'ad-psgd.py line 375 : {gossip_meter}')
                    gossip_meter.update(time.time() - bt)
                    #logger.debug(f'ad-psgd.py line 376 : {gossip_meter}')
    ...
    def _gossip_target(dist_config, gossip_enable_flag, train_write_flag,
                       gossip_read_flag, gossip_update_flag, gossip_lr,
                       gossip_lock, gossip_params, gossip_grads, gossip_device_buffer, gossip_stream):
        """ Gossip thread, which performs push-sum on model params """
        with torch.no_grad():
