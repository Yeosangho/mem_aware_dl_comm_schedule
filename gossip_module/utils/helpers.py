...
def flatten_tensors_grad(tensors):
    if len(tensors) == 1:
        return tensors[0].grad.view(-1).clone()
    flat = torch.cat([t.grad.view(-1) for t in tensors], dim=0)
    return flat
...
def unflatten_tensors_grad(flat, tensors):
    """
    View a flat buffer using the sizes of tensors. Assume that tensors are of
    same dense type, and that flat is given by flatten_dense_tensors.
    Arguments:
    ...
    for tensor in tensors:
        numel = tensor.numel()
        outputs.append(flat.narrow(0, offset, numel).view_as(tensor.grad))
        offset += numel
    return tuple(outputs)
