...
from all_reduce_bucketer import AllReduceBucketer
...
class DataParallel_Custom(nn.Module):
    ...
    def __init__(
        ...
        done_counts=None, partition_counts=None, locks=None, conditions=None, forward_conditions=None, memory_record=None, comm_stream=None
    ):
    ...
    def _setup_streams(self) -> None:
        ...
        #self._allreducer = AllReduceBucketer(self.bucket_cap_mb)
        ...
        for n, m in self.named_modules():
            if n != "" and isinstance(m, DataParallel_Custom):
                m._streams = self._streams
                #m._allreducer = self._allreducer

    def _reset_lazy_init(self) -> None:
        """Reset instance so :func:`_lazy_init` will run on the next forward."""
        self._is_root: Optional[bool] = None
        self._streams: Dict[str, torch.cuda.Stream] = {}
        self._allreducer: Optional[ReduceScatterBucketer] = None
...
#
    ...
    def _post_backward_hook(self, param: Parameter, *unused: Any) -> None:
        ...
        #self._allreducer.all_reduce_async(param.grad, group=self.process_group)
    ...
    def _queue_wait_for_post_backward(self) -> None:
        ...
        if not self._post_backward_callback_queued:
            print("_queue_wait_for_post_backward")
            self.assert_state([TrainingState.IDLE])
    ...
    def _wait_for_post_backward(self) -> None:
        ...
        def _finalize_parameters(dp_module: DataParallel_Custom) -> None:
            """Helper used below on all fsdp modules."""
            for p in dp_module.params:
...
#
        ...
        for m in self.modules():  # includes self
            if isinstance(m, DataParallel_Custom):
    ...
    def set_gradient_divide_factors(self, pre: float, post: float, recursive: bool) -> None:
        ...
        if recursive:
            for module in self.modules():
                if isinstance(module, DataParallel_Custom) and module != self:
    ...
    def apply(self, fn: Callable[[nn.Module], None]) -> "DataParallel_Custom":
        """
        ...
        if is_uninitialized and self._is_root:
            for module in self.modules():
                if isinstance(module, DataParallel_Custom):
    ...
    def _cast_buffers(
        ...
        if memo is None:
            memo = set()
        for module in self.modules():
            if module is not self and isinstance(module, DataParallel_Custom):
    ...
    def local_state_dict(self, *args: Any, **kwargs: Any) -> Any:
        ...
        with contextlib.ExitStack() as stack:
            ...
            return DataParallel.state_dict(self, *args, **kwargs)
...
def _post_state_dict_hook(
    module: DataParallel_Custom, state_dict: "OrderedDict[str, torch.Tensor]", prefix: str, *args: Any
    ...
    replace_by_prefix_(state_dict, prefix + "_dp_wrapped_module.", prefix)
    return state_dict
...
def _pre_load_state_dict_hook(
    ...
    replace_by_prefix_(state_dict, prefix, prefix + "_dp_wrapped_module.")


def _clean_path(path: str) -> str:
    """ Remove FSDP related wrapper modules from a given state dict key str path. """
    return ".".join([split for split in path.split(".") if split not in {"_dp_wrapped_module", "_fpw_module"}])
...
def auto_wrap_bn(
    ...
    # Wrap it.
    with (
        enable_wrap(config_auto_wrap_policy, wrapper_cls=DataParallel_Custom) if wrap_it else contextlib.suppress()
    ):
