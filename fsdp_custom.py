...
class FullyShardedDataParallel(nn.Module):
    ...
    def __init__(
        ...
        rs_locks=None, ag_locks=None, ag_fsdp_locks=None,

        rs_conditions=None, ag_conditions=None, ag_fsdp_conditions=None,

        forward_conditions=None, backward_conditions=None,

        lazy_init_locks=None, lazy_init_conditions=None,
        ...
        param_name_groups = [[n] for n in self.param_names]
        if self.flatten_parameters:
            to_be_flatten_params = [params]
            non_flatten_params = []
            param_name_groups = [self.param_names]
    ...
    def _wait_for_previous_optim_step(self) -> None:
        ...
        """
        #print("wait for previous optim step")
    ...
    def _acquire_lock(self, lock, condition):
        lock.acquire()
        with condition:
    ...
    def forward(self, *args: Any, **kwargs: Any) -> torch.Tensor:
        ...
        #print("before lazy init lock")
        #if(self.is_first_itr == True):
        #    for param in self.params :
        #        self._acquire_lock(self._lazy_init_locks[param], self._lazy_init_conditions[param])
        self._lazy_init()    
        #print("after lazy init lock")
        ...
        if self._is_root and self.mixed_precision:
            #print("cast_floats_to_right_precision")
        ...
        #print("before rebuild full params")
        #if(self.is_first_itr == True):
        ...
        for p in self.params : 
            self._acquire_lock(self._ag_fsdp_locks[p], self._ag_fsdp_conditions[p])
        #print(self.param_names)
        if(self.is_first_itr == True):
            self.is_first_itr = False
        return outputs
    ...
    def _register_pre_backward_hooks(self, outputs: Any) -> Any:
        ...
        def _pre_backward_hook(*unused: Any) -> None:
            ...
            #for p in self.params : 
            #    self._wait_unlock(self._ag_fsdp_locks[p], self._backward_conditions[p])
    ...
    def _post_backward_hook(self, param: Parameter, *unused: Any) -> None:
        ...
        #print(f"post backward : {param.shape} {torch.sum(param)}")
        #print(f"post backward grad : {param.grad.shape} {torch.sum(param.grad)}")
        ...
        #param.grad.data = torch.zeros_like(grad_chunks[0]).type(param.grad.dtype).to(param.device)
        #param.grad = None      
        #self._post_reduction_hook(param, param.grad.data)
        #param.grad = None
        for m in self.modules():  # includes self
            if isinstance(m, FullyShardedDataParallel):
                #print(f"before finalize parameters  {torch.cuda.memory_allocated() / 1024 /1024}") 
                #_finalize_parameters(m)
                #print(f"after finalize parameters  {torch.cuda.memory_allocated() / 1024 /1024}") 
                m._pre_backward_hook_has_run = False

        #self._locks[param].acquire()
        #with self._conditions[param]:
        #    self._conditions[param].notify_all()
        self._acquire_lock(self._rs_locks[param], self._rs_conditions[param])

        if not param._is_sharded:
    ...
    def _wait_for_post_backward(self) -> None:
        ...
        def _finalize_parameters(fsdp_module: FullyShardedDataParallel) -> None:
            ...
            #print(f"finalize parameters  {torch.cuda.memory_allocated() / 1024 /1024}") 
...
##
        ...
        for m in self.modules():  # includes self
            if isinstance(m, FullyShardedDataParallel):
                ...
                if m._is_root:
                    ...
                    self._post_backward_callback_queued = False
    @torch.no_grad()
    def _rebuild_full_params_zero3(self, force_full_precision: bool = False) -> Optional[List[Tuple[torch.Tensor, bool]]]:
        """
        ...
        def update_p_data(custom_output_tensor: Optional[torch.Tensor] = None) -> None:
            ...
            #print(f"rebuild data {p.data.shape} {torch.sum(p.data)}")
        ...
        for p in self.params:
            ...
            if not p._is_sharded:  # e.g., when world_size == 1
                ...
                if hasattr(dist, "_all_gather_base") and enable_nccl_base_collectives:
                    ...
                    #print(f"allgather  {output_tensor.shape}")
                    dist._all_gather_base(output_tensor, p_data)
                    p_data = None
                    #print(f"!!!!! {output_tensor.shape}, {torch.sum(p_data)}")
                else:
                    chunks = list(output_tensor.chunk(self.world_size))
                    #print("?????")
                    dist.all_gather(chunks, p_data)
                #print(f"allgather  {output_tensor.shape}")
            #    torch.cuda.synchronize()
                #dist.barrier()
                #output_tensor = torch.flatten(torch.stack(chunks))
    ...
    def _rebuild_full_params(self, force_full_precision: bool = False) -> Optional[List[Tuple[torch.Tensor, bool]]]:
        ...
        #len_locks = len(self._locks)
        ##print(p)
        #if not list(self._locks.values())[len_locks-1].locked():
        #    #if len(self._handlequeue) > 0 :
        #    #    #print(f"pre forward {self._parameter_names[p]}")
        #    #    handle = self._handlequeue.pop(0)
        #    #    handle.wait()
        ...
        #    None 
        #else :
        #    with list(self._forward_conditions.values())[len_locks-1] :
        #        #print(f"wait!!{self.param_names}")
        #        list(self._forward_conditions.values())[len_locks-1].wait()
        #        #print("wait end!!")
        #        #if len(self._handlequeue) > 0 :
        #        #    #print(f"pre forward {self._parameter_names[p]}")
        #        #    handle = self._handlequeue.pop(0)
...
#
        ...
        def update_p_data(custom_output_tensor: Optional[torch.Tensor] = None) -> None:
            ...
            p.data = p.data[: p._orig_size.numel()].view(p._orig_size)
            #print(f"after update p_data {p.data.shape} {torch.sum(p.data)}")     
            #print(f"rebuild data {p.data.shape} {torch.sum(p.data)}")
        ...
        with torch.cuda.stream(self.comm_stream):
            if self.mixed_precision and not force_full_precision:
                self._cast_fp32_param_shards_to_fp16()
            for p in self.params:
                if p._full_param_padded.storage().size() !=  p._full_param_padded.size().numel():
                    if not p._is_sharded:  # e.g., when world_size == 1
                        update_p_data()
                    else:
                        # If self.move_params_to_cpu and force_full_precision, we need to cast
                    ...
                    #output_tensor = p_data.new_zeros(p_size)
                    #print(f"{p._full_param_padded.shape}")
                    output_tensor = p._full_param_padded
                    #torch.cuda.synchronize()
                    #dist.barrier()
                    #output_tensor = torch.flatten(torch.stack(chunks))
